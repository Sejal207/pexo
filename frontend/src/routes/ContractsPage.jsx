...
export const ContractsPage = () => {
  ...
  const filteredEmployeeName = employeeId ? contracts[0]?.employeeName : null;
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      ...
      {employeeId && <div className="flex items-center gap-2 text-sm text-slate-400">Showing contracts for <span className="font-medium text-white">{filteredEmployeeName ?? 'this employee'}</span><button type="button" onClick={() => navigate('/contracts')} className="text-indigo-400 hover:text-indigo-300">Clear filter</button></div>}
